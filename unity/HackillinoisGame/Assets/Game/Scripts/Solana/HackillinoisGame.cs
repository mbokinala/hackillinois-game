...
using HackillinoisGame;
using HackillinoisGame.Program;
using HackillinoisGame.Errors;
using HackillinoisGame.Accounts;

namespace HackillinoisGame
{
    namespace Accounts
    {
    ...
    namespace Errors
    {
        public enum HackillinoisGameErrorKind : uint
        {
    ...
    public partial class HackillinoisGameClient : TransactionalBaseClient<HackillinoisGameErrorKind>
    {
        public HackillinoisGameClient(IRpcClient rpcClient, IStreamingRpcClient streamingRpcClient, PublicKey programId) : base(rpcClient, streamingRpcClient, programId)
        {
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.HackillinoisGameProgram.InitPlayer(accounts, levelSeed, programId);
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.HackillinoisGameProgram.ChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }
        ...
        {
            Solana.Unity.Rpc.Models.TransactionInstruction instr = Program.HackillinoisGameProgram.SuperChopTree(accounts, levelSeed, counter, programId);
            return await SignAndSendTransaction(instr, feePayer, signingCallback);
        }

        protected override Dictionary<uint, ProgramError<HackillinoisGameErrorKind>> BuildErrorsDictionary()
        {
            return new Dictionary<uint, ProgramError<HackillinoisGameErrorKind>>{{6000U, new ProgramError<HackillinoisGameErrorKind>(HackillinoisGameErrorKind.NotEnoughEnergy, "Not enough energy")}, {6001U, new ProgramError<HackillinoisGameErrorKind>(HackillinoisGameErrorKind.WrongAuthority, "Wrong Authority")}, };
        }
    }
    ...
    namespace Program
    {
        ...
        public static class HackillinoisGameProgram
        {
